...
from Test_policy.test_option import CreateOptions
...
def run_learned_policy_feedback(iter_num, trial_num):
    ...
    create_options = CreateOptions([2, 5], 3, trial_num=trial_num)
    option, option_states, option_rewards = create_options.create_options()
    # -----------------

    next_state = option_states[-1]
...
def run_learned_policy_update(iter_num, trial_num):
    exp_num = 3  # DO NOT CHANGE, THIS IS FIXED!!
    ...
    current_trajectory = np.load('data/current_trajectory.npy', allow_pickle=True)
